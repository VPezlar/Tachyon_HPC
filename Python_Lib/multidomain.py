...
def Vert_Interface_C0(A, N_eq, Discretization, Dimensions, Ver_interfaces_C0, grids, q):
    ...
    for i in range(len(Ver_interfaces_C0[:, 0])):
        ...
        grid1 = grids[Ver_interfaces_C0[i, 1] - 1]
        if len(grid1.shape) > 1:
            grid1 = grid1[:, 0]
        grid2 = grids[Ver_interfaces_C0[i, 0] - 1]
        if len(grid2.shape) > 1:
            grid2 = grid2[:, -1]

        INTERP_Y_C0 = FDq.FDq_Int(grid1, grid2, q)
...
def Vert_Interface_C1(A, N_eq, Discretization, Dimensions, Ver_Interfaces_C1, DGL_Z, grids, q):
    ...
    for i in range(len(Ver_Interfaces_C1[:, 1])):
        ...
        grid1 = grids[Ver_Interfaces_C1[i, 0] - 1]
        if len(grid1.shape) > 1:
            grid1 = grid1[:, -1]
        grid2 = grids[Ver_Interfaces_C1[i, 1] - 1]
        if len(grid2.shape) > 1:
            grid2 = grid2[:, 0]

        INTERP_Y_C1 = FDq.FDq_Int(grid1, grid2, q)
        ...
        for k in range(N_eq):
            ...
            Inter = scipy.sparse.lil_matrix(np.zeros((Dim[Ver_Interfaces_C1[i, 1] - 1, 0], D1.shape[0])))
            for m in range(Dim[Ver_Interfaces_C1[i, 1] - 1, 0]):
                for n in range(Dim[Ver_Interfaces_C1[i, 0] - 1, 0]):
                    Inter[m, :] = Inter[m, :] + INTERP_Y_C1[m, n] * D1[Dim[Ver_Interfaces_C1[i, 0] - 1, 1] * (n + 1) - 1, :]
            ...
            for l in range(Dim[Ver_Interfaces_C1[i, 1] - 1, 0]):
                ...
                A[verLoc + verMove, horStart:horEnd] = - Inter[l, :]
    ...
    if Hor_Interfaces_C1.size == 0:
        ...
        for i in range(len(Hor_Interfaces_C1[:, 1])):
            ...
            for k in range(N_eq):
                ...
                Inter = scipy.sparse.lil_matrix(np.zeros((Dim[Hor_Interfaces_C1[i, 1] - 1, 1], D1.shape[0])))
                for m in range(Dim[Hor_Interfaces_C1[i, 1] - 1, 1]):
                    for n in range(Dim[Hor_Interfaces_C1[i, 0] - 1, 1]):
                        Inter[m, :] = Inter[m, :] + INTERP_Z_C1[m, n] * D1[((Dim[Hor_Interfaces_C1[i, 0]-1, 0] - 1) * Dim[Hor_Interfaces_C1[i, 0]-1, 1]) + n, :]
