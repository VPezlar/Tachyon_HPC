...
N_wave_Iter     = int(config['STABILITY']['N_wave_Iter'])
...
for d_Wave in range(N_wave_Iter):
    ...
    del dFDq_eta1, dFDq_eta2, dFDq_xi1, dFDq_xi2
    del dx_1, dxx_1, dy_1, dyy_1
    ...
    wave_num  = np.double(config["STABILITY"]["wave_num"]) + delta_wave_Iter * d_Wave
    ...
    PBCz = [PBCO.FormPBC(RHO1, T1, D1_n), PBCO.FormPBC(RHO2, T2, D2_n)]
    PBCy = [PBCO.FormPBC(RHO1, T1, D1_y), PBCO.FormPBC(RHO2, T2, D2_n)]

    del D1_n, D2_n
    del D1_y, D2_y, D1_x, D2_x
    ...
    # 1. Define the output path using Path logic
    case_number = wave_num
    base_path = Path(CWD) / ".." / "Output" / "CSV" / "Eigenvalues" / str(case_number)
    file_path = base_path / "Eigenvalues.csv"

    # 2. Create the directory structure if it doesn't exist
    # This creates ../Output, ../Output/CSV, etc. automatically
    base_path.mkdir(parents=True, exist_ok=True)

    # 3. Write the file
    with open(file_path, 'w') as EIGEN:
        for i in range(N_eig):
            # f-strings are generally cleaner and faster than % formatting
            # This formats to 15 decimal places with a space for sign
            real_part = f"{eig_val[i, 0].real: .15f}"
    ...
    for i in range(N_Out):
        eig = i
        SD_ID_1 = N_eq * ((Nx_1 + 1) * (Ny_1 + 1))
        SD_ID_2 = SD_ID_1 + N_eq * ((Nx_2 + 1) * (Ny_2 + 1))
        ...
        name = CWD + "/../Output/VTK/Eigenvectors/" + str(wave_num) + "/" + str(i) + ".vtk"
